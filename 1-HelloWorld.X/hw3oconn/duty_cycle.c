...
//Based on discussion section code
...
//void __attribute__((__interrupt__,__auto_psv__)) _INT0Interrupt(void) //comment out for polling
//{
//    _INT0IF = 0;
//
...
void setup(void)
{
    ...
    _INT0EP = 0;
    _INT0IF = 0;
//    _INT0IE = 1; // comment out for interrupt
    _INT0IP = 6;  // higher priority compared to T1
    // simulate the code, put breakpoint in the T1 ISR.
    // in the middle of the T1 ISR, use stimulus to
    // cause INT0 interrupt. You see that the code
    // does break T1 ISR to handle INT0 ISR.
    // You can change _INT0IP=2 and repeat the same
    // experiments. You will see that T1 ISR does 
    // *NOT* get interrupted to service INT0 ISR.
